...
kingroosterSONG = "@kingroosterbotMP3"
kingroosterVSONG = "@kingroosterbotMP4"
...
async def download_song(v_url):
    ...
    rkp = await v_url.reply("`എടുത്തോണ്ട് ഇരിക്കുവല്ലേ ..... ദൃതി വയ്ക്കാതെ....`")
    if not url:
    ...
    type = "audio"
    await rkp.edit("`മോഷ്ടിക്കാൻ തുടങ്ങുന്നു ...`")
    if type == "audio":
        opts = {
            "format": "bestaudio",
            "addmetadata": True,
    ...
    if song:
        ...
        await y.forward_to(kingroosterSONG)
        os.system("rm -rf *.mp3")
...
async def download_video(v_url):
    ...
    if video:
        ...
        await y.forward_to(kingroosterVSONG)
        os.system("rm -rf *.mp4")
        os.system("rm -rf *.webp")
...
**Also check out - **
 @MisskingroosterRobotMP3
 @MisskingroosterRobotMP4
"""
