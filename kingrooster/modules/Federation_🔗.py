...
import kingrooster.modules.sql.feds_sql as sql
...
# ME @Misskingrooster_Robot has also done a lot of hard work to rewrite this in telethon so add this line as a credit. Please don't remove this if you somewhat respect me.
...
async def _(event):
    ...
    if fednam:
        ...
        if not x:
            await event.reply(
                "Can't create federation!\nPlease contact @kingroosterbotSupport if the problem persists."
            )
            return
...
async def _(event):
    ...
    if args:
        ...
        if not x:
            await event.reply(
                "Failed to join federation! Please contact @MisskingroosterRobotSupport should this problem persist!"
            )
            return
...
async def _(event):
    try:
        ...
        if fban:
            ...
            if not x:
                await event.reply(
                    "Failed to ban from the federation! If this problem continues, contact @MisskingroosterRobotSupport."
                )
                return
...
async def _(event):
    try:
        ...
        if args:
            ...
            if not x:
                await event.reply(
                    "There was an error while setting federation rules!\nPlease go to @MisskingroosterRobotSupport to report this."
                )
                return
    ...
    try:
        ...
        if args:
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "kingrooster_fbanned_users.json"
                await tbot.send_file(
                    event.chat_id,
                    file=output,
                    filename="kingrooster_fbanned_users.json",
...
__help__ = """
 ...
 - /fban (<user>|<reason>): Fed bans a user. Syntax: `fban 12345 | testing`, `fban @MisskingroosterRobot | testing`.
